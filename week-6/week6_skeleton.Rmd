...
```{r, include=FALSE}
load(file = here::here("week-6", "rf_tune.rda"))
```
...
```{r, include=FALSE}
load(file = here::here("week-6", "xgb_tune.rda"))
```
...
test_predict2_xbg = predict(train_fit_xgb, test, type = "prob") %>% #get testing prediction
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))
```
...
- not that different in accuracy, but boosted is slightly better so we'd choose that one
...
test_roc_auc_xgb = roc_curve(test_predict2_xbg, habitat, .pred_bathydemersal, .pred_bathypelagic, .pred_benthopelagic, .pred_demersal, .pred_pelagic, `.pred_pelagic-neritic`, `.pred_pelagic-oceanic`, `.pred_reef-associated`)
```
