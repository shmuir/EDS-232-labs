...
set.seed(258)
forest_fold <- vfold_cv(forest_training, strata = cover_type)
...
# augment 
augment(svm_linear_fit, new_data = forest_testing) %>%
...
augment(svm_rbf_fit, new_data = forest_testing) %>%
  conf_mat(truth = cover_type, estimate = .pred_class)

#We can examine our model's performance using ROC and AUC
augment(svm_rbf_fit, new_data = forest_testing) %>%
  roc_curve(truth = cover_type, .pred_1) %>%
  autoplot()

augment(svm_rbf_fit, new_data = forest_testing) %>%
  roc_auc(truth = cover_type, .pred_1)
```
