...
tracks_split <- initial_split(all_tracks_modeling, prop = 0.7)
...
tracks_recipe <- recipe(name ~ ., data = tracks_train) %>%
 # step_dummy(all_nominal()) %>%
  step_normalize(all_numeric()) %>% #normalize numeric to make sure scale is okay
  prep()
```
...
```{r}
tracks_dt1 <- rpart(formula = name ~ .,
                    data    = tracks_train,
                    method  = "anova")

rpart.plot(tracks_dt1)
plotcp(tracks_dt1)
```
...
vip(tracks_dt3, num_features = 40, geom = "point")
```
