...
```{r}
eel_spec_lr_tune <- boost_tree(trees = 3000, learn_rate = tune()) %>%
  set_engine("xgboost") %>%
...
eel_stochastic_spec <- 
  parsnip::boost_tree(
    ...
    learn_rate = select_best(eel_cv_tune, metric = "roc_auc")$learn_rate, #the rate at which the bosting algorithm adapts from iteration-to-iteration
...
xgboost_stochastic_params <- 
  dials::parameters(finalize(mtry(), select(eel_training, -Angaus)), #mtry values will range from 1 to the number of predictors included in the model
...
roc_obj <- pROC::roc(true_labels, predictions)
...
**From my model, summer air temperature (SegSumT) and distance to coast (DSDist) were the most important predictors. Summer air temperature was also the most important predictor for the model from the paper. In general, sites where Angaus was detected were likely to be warmer and closer to the coast than areas where Angaus was not detected.**
